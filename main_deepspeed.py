...
if args.pipeline_parallel:
    ...
    hf_model = PipelineModule(
        layers=model_layers,
        num_stages=num_gpus,  # e.g., 2, 4, 8, etc.
        partition_method='parameters'  # or 'uniform', 'type:transformer', etc.
    )
...
#     print(f"DEBUG {name} is on device: {param.device}") 


def print_device_map_old(model):
    """
    Prints the device map of the model's parameters.
    """
    layer_output = {}
...
# for name, param in deep_speed_model_engine.named_parameters():
#     print(f"DEBUG local_rank: {local_rank}, name: {name} is on device: {param.device}")
#     dist.barrier()


print_device_maps(deep_speed_model_engine, local_rank)
# print("DEBUG: type(deep_speed_model_engine.named_parameters()): ", type(deep_speed_model_engine.named_parameters()))  # This will show the device map of the model
# print("DEBUG: deep_speed_model_engine.named_parameters(): ", deep_speed_model_engine.named_parameters())  # This will show the device map of the model
