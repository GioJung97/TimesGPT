...
test_sampler = DistributedSampler(test_dataset, shuffle=True, num_replicas=num_gpus, rank=args.local_rank)
...
if args.do_test:
    ...
    if args.local_rank == 0:
        ...
        metrics_dict = {}       
        metrics_dict["avg_test_loss"] = avg_loss
        ...
        with open(os.path.join(output_dir, experiment_name +".csv"), 'w') as f:
            for i,filename in enumerate(ground_truth_captions_dict):
                f.writelines(filename + "," + predicted_captions[i] + "," + ground_truth_captions[i] + "\n")
        ...
        with open(os.path.join(output_dir, experiment_name + ".html"), 'w') as f:
            ...
            for i,filename in enumerate(ground_truth_captions_dict):
                ...
                f.write(f"<p>{i}, {filename}<br>Predicted Caption: {predicted_captions[i]}<br>Ground-Truth Caption: {ground_truth_captions[i]}</p><br>\n")
