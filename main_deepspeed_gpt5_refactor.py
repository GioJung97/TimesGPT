...
def build_arg_parser():
    ...
    parser.add_argument('-lr', '--learning_rate', type=float, default=5e-5) # 5e-7 too small
    ...
    parser.add_argument('--debug_label_stats', action='store_true')  # Print padding / valid token stats periodically
    parser.add_argument('--test_streaming', action='store_true')     # Stream test decoding item-by-item to reduce memory
...
def greedy_decode_pipeline(engine, pixel_values, labels, metadata, tokenizer, max_len=50, ctx_len=1024):
    ...
    for _ in range(max_len):
        ...
        # Inference: disable loss computation & loss broadcast to avoid extra collectives.
        batch_iter = iter(RepeatingLoader([((pixel_values, dec_in, metadata), labels)]))
        try:
            _, out = engine.eval_batch(batch_iter, return_logits=True, compute_loss=False, bcast_loss=False)
        except Exception as e:
            print(f"[greedy_decode_pipeline][Rank {dist.get_rank()}] eval_batch failed: {e}")
            raise
        ...
        if is_last:
            ...
            # Force finished sequences to emit EOS to keep lengths aligned
            next_tok = torch.where(finished, torch.full_like(next_tok, tokenizer.eos_token_id), next_tok)
        else:
...
def sample_decode_pipeline(engine, pixel_values, labels, metadata, tokenizer, max_len=50, ctx_len=1024, top_k=50, top_p=0.9, temperature=1.0, no_repeat_ngram_size: int = 0):
    ...
    for _ in range(max_len):
        ...
        try:
            ...
            print(f"[sample_decode_pipeline][Rank {dist.get_rank()}] eval_batch failed: {e}")
            raise
        ...
        if is_last:
            ...
            next_tok = next_tok.to(device, non_blocking=True)
            # Keep finished sequences emitting EOS
            next_tok = torch.where(finished, torch.full_like(next_tok, tokenizer.eos_token_id), next_tok)
        else:
            next_tok = torch.empty(B, dtype=torch.long, device=device)
...
def beam_search_decode_pipeline(engine, pixel_values, labels, metadata, tokenizer, max_len=50, ctx_len=1024, num_beams=4, length_penalty=1.0, no_repeat_ngram_size: int = 0, temperature: float = 1.0):
    ...
    for step in range(max_len):
        ...
        try:
            ...
            print(f"[beam_search_decode_pipeline][Rank {dist.get_rank()}] eval_batch failed: {e}")
            raise
...
# Important: do not override n_positions when loading from pretrained to avoid wpe size mismatch.
# We'll constrain runtime context via decoding max_len/ctx_len instead.
...
if args.fresh_weights:
    ...
    # Fresh weights allow overriding n_positions if desired; for now keep pretrained config's positions for stability.
...
if args.test_streaming:
    ...
    test_bs = ds_config["train_micro_batch_size_per_gpu"]
test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=test_bs, collate_fn=default_collate, drop_last=True)
...
if args.do_train:
    ...
    for epoch in range(args.num_epochs):
        ...
        print(f"Starting training epoch {epoch + 1}/{args.num_epochs}...")
        ...
        for step in range(steps_per_epoch):
            print("DEBUG: Train step:", step, " rank:", dist.get_rank(), " world size:", dist.get_world_size())
            loss = engine.train_batch()

            print("DEGBUG: Train step loss:", loss.item())
            if dist.get_rank() == (dist.get_world_size() - 1):
...
# Testing + decoding
print("DEBUG: Waiting for all ranks to finish training/validation before testing...")
dist.barrier()
print("DEBUG: All ranks ready for testing...")

if args.do_test:
    engine.eval()
    # Use plain DataLoader, no RepeatingLoader, no distributed syncs
    strategy = 'greedy'
    ...
    if engine.is_last_stage():
        ...
        try:
            ...
            print(f"WARN: failed to init qualitative report: {e}")
    ...
    for batch in test_dataloader:
        ...
        labels = batch[0][1]
        metadata = batch[0][2]
        if args.num_qualitative > 0 and num_written >= args.num_qualitative:
            break
        try:
            if strategy == 'greedy':
            ...
            print(f"[test_loop][Rank {dist.get_rank()}] Decoding failed at batch {num_written}: {e}")
            continue
        ...
        if engine.is_last_stage():
            ...
            for i in range(B):
                ...
                try:
                    ...
                    print(f"WARN: failed to append qualitative row: {e}")
                num_written += 1

# Metrics only on last stage (already streamed CSV); compute BLEU/WER aggregates.
if engine.is_last_stage():
        if BLEUScore is not None:
            try:
                ...
                print(f"WARN: BLEU compute failed: {e}")
        if WordErrorRate is not None:
            try:
                ...
                print(f"WARN: WER compute failed: {e}")
        # Optional: METEOR/ROUGE/SPICE disabled in streaming mode to avoid accumulation.
